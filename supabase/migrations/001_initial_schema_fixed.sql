...
-- Simple text search indexes (Korean text search is not available in Supabase)
-- These indexes will help with LIKE '%text%' searches
CREATE INDEX idx_cases_title ON cases USING gin(title gin_trgm_ops);
CREATE INDEX idx_cases_content ON cases USING gin(content gin_trgm_ops);
...
CREATE INDEX idx_community_posts_title ON community_posts USING gin(title gin_trgm_ops);
CREATE INDEX idx_community_posts_content ON community_posts USING gin(content gin_trgm_ops);
CREATE INDEX idx_announcements_title ON announcements USING gin(title gin_trgm_ops);
CREATE INDEX idx_announcements_content ON announcements USING gin(content gin_trgm_ops);
