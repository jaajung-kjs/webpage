...
### 1. DB 수정 원칙 - "DB는 신성하다"
- **🚨 CRITICAL: 함수 생성 전 MANDATORY 체크 🚨**
  1. **FIRST**: `SELECT routine_name FROM information_schema.routines WHERE routine_name LIKE '%키워드%' AND routine_schema = 'public';`
  2. **SECOND**: 기존 함수 있으면 절대 새로 만들지 말고 기존 함수 사용
  3. **THIRD**: 정말 없을 때만 생성, 그것도 3번 재확인 후
- DB 수정이 필요하다고 생각되면:
